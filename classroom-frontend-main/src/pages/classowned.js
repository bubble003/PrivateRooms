...
console.log(classData);
  ...
  const deleteMember = async (memberId) => {
    try {
      ...
      await axios.delete(`${process.env.REACT_APP_PATH_URL}/class/${id}/participants/${memberId}`, config);
  ...
  useEffect(() => {
    const fetchClassData = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_PATH_URL}/class/owner/${id}`, config);
  ...
  const updateClassDetails = async (updatedDetails) => {
    try {
      ...
      const response = await axios.patch(`${process.env.REACT_APP_PATH_URL}/class/${id}`, updatedDetails, config);
